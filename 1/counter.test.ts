...
describe("Counter", () => {
  test("initializes with correct position", () => {
    ...
    const counter100 = new Counter(100, 50);
    expect(counter100.position).toBe(50);
  });

  describe("R", () => {
    test("can count up", () => {
      const counter = new Counter(100, 50);
      expect(counter.position).toBe(50);
  
      expect(counter.count("R20")).toBe(70);
      expect(counter.position).toBe(70);
    });
  
    test("counts to 0 when going to limit", () => {
      const counter = new Counter(100, 50);
      expect(counter.position).toBe(50);
  
      expect(counter.count("R50")).toBe(0);
      expect(counter.position).toBe(0);
    });
  
    test("counts over 0 when going over limit", () => {
      const counter = new Counter(100, 50);
      expect(counter.position).toBe(50);
  
      expect(counter.count("R51")).toBe(1);
      expect(counter.position).toBe(1);
    });
  
    test("counts to 0 when going to limit plus multiples of limit", () => {
      const counter = new Counter(100, 50);
      expect(counter.position).toBe(50);
  
      expect(counter.count("R150")).toBe(0);
      expect(counter.position).toBe(0);
    });
  
    test("emits lap event when counting to 0 when going to limit", () => {
      let laps = 0;
      const lapHandler = (lapped: number) => laps+=lapped;
      const counter = new Counter(100, 50, lapHandler);
      expect(counter.position).toBe(50);
    ...
    test("emits lap event when going to limit plus multiples of limit", () => {
      let laps = 0;
      const lapHandler = (lapped: number) => laps+=lapped;
      const counter = new Counter(100, 50, lapHandler);
      expect(counter.position).toBe(50);
  ...
  describe("L", () => {
    test("counts down", () => {
      ...
      expect(counter.count("L2")).toBe(3);
    });
    
    test("counts down to limit", () => {
      const counter = new Counter(10, 5);
      expect(counter.count("L5")).toBe(0);
    });

    test("counts down under limit", () => {
      const counter = new Counter(100, 5);
      expect(counter.count("L6")).toBe(99);
    });

    test("counts down to limit when over full rotation", () => {
      const counter = new Counter(10, 5);
      expect(counter.count("L15")).toBe(0);
    });

    test("counts down under limit when over full rotation", () => {
      const counter = new Counter(10, 5);
      expect(counter.count("L16")).toBe(9);
    });

    test("emits lap event when counting down to limit", () => {
      let laps = 0;
      const lapHandler = (lapped: number) => laps+=lapped;
      const counter = new Counter(100, 50, lapHandler);
      expect(counter.position).toBe(50);
  
      expect(counter.count("L50")).toBe(0);
      expect(laps).toBe(1);
    });

    test("emits lap event when counting down to limit and multiples of limit", () => {
      let laps = 0;
      const lapHandler = (lapped: number) => laps += lapped;
      const counter = new Counter(100, 50, lapHandler);
      expect(counter.position).toBe(50);
  
      expect(counter.count("L150")).toBe(0);
      expect(laps).toBe(2);
    });
  });
  ...
  describe("adventofcode", () => {
    test("problem sample", () => {
      const inputStr = "L68,L30,R48,L5,R60,L55,L1,L99,R14,L82";
      const inputArr = inputStr.split(",")
      let laps = 0;
      const lapHandler = (lapped: number) => laps+=lapped;
      const counter = new Counter(100, 50, lapHandler);
      for (const key in inputArr) {
        const element = inputArr[key];
        if(element) {
